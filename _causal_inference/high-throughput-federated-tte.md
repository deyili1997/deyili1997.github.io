...
For full paper-by-paper methodological readings, see [high-throughput AD drug repurposing]({{ "/rwe/high-throughput-ad-target-trial-emulation/" | relative_url }}) and [federated target trial emulation]({{ "/rwe/federated-target-trial-emulation/" | relative_url }}). A related extension, [EmulatRx]({{ "/rwe/emulatrx-agentic-trial-design/" | relative_url }}), uses AI agents to coordinate trial design and emulation. All three are collected in the [Pharmacoepidemiology paper notes]({{ "/real-world-evidence/" | relative_url }}#area-paper-notes-title).
