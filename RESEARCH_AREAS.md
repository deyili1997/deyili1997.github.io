...
The website has four peer topics, defined in `_data/research_areas.json`. Display names were updated to Medical Informatics and Pharmacoepidemiology; internal topic IDs, existing URLs and publication source folders are retained for compatibility:
...
Pharmacoepidemiology uses `real-world-evidence` as its internal topic ID and `/real-world-evidence/` as its canonical hub. The former `/rwe-generation/` hub redirects to it, and `/publication/#rwe-generation` remains an anchor alias for `/publication/#real-world-evidence`. The source publication folder remains `Publications/RWE/` and maps to Pharmacoepidemiology.

Medical Informatics uses `healthcare-ai` as its internal topic ID and `/healthcare-ai/` as its canonical hub. The former `/predictive-modeling/` hub redirects to it, and `/publication/#predictive-modeling` remains an anchor alias for `/publication/#healthcare-ai`. Original paper titles and post URLs are retained. The source publication folder remains `Publications/PM/` and maps to Medical Informatics.

Causal Inference is a study collection within Pharmacoepidemiology. The Pharmacoepidemiology hub shows every foundation note directly under `#area-learning-title`, without pagination or collapsed groups. `_data/rwe_learning.json` defines five consecutive learning stages; `start` is inclusive and `end` is exclusive, with the last stage open-ended. Keep boundaries contiguous when extending the route. Notes are sorted numerically by `order`; displayed labels preserve companion steps such as 14.1 and 14.2. The guide remains at `/causal-inference/` as an additional reading aid and all existing note URLs are retained. Note breadcrumbs return to the Pharmacoepidemiology directory.

The Pharmacoepidemiology hub puts foundations and paper readings side by side on wide screens and stacks them on smaller screens. On-page links provide direct access to either section. Individual articles retain the site's narrower reading width. Homepage links and counts are generated from the two collections.

### Adding a Pharmacoepidemiology paper reading
...
- All foundation notes and paper readings remain visible on screen and in print. The Pharmacoepidemiology directory has no local search, filter, sort, or reset controls and requires no directory JavaScript. The global site Search remains available.
...
Existing blog posts are Medical Informatics learning notes. They retain their URLs, search entries and the legacy paginated archive. The post default is `research_area: healthcare-ai`; explicitly override that field when a future post belongs to another topic. Learning notes are separate from authored publications.
...
The homepage includes a simple diagram of three research directions with four separate one-way arrows: Medical Informatics → Clinical Pharmacology, Medical Informatics → Pharmacoepidemiology, Clinical Pharmacology → Pharmacoepidemiology, and Pharmacoepidemiology → Clinical Pharmacology. The two arrows between Clinical Pharmacology and Pharmacoepidemiology use distinct parallel lines, each with one arrowhead. Its inline SVG is in `_includes/research-relationships.html`.
